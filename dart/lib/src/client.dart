...
class CallingcodeValidationException implements Exception {
  final List<String> errors;

  CallingcodeValidationException(this.errors);

  @override
  String toString() => 'CallingcodeValidationException: ${errors.join("; ")}';
}
...
/// Country Calling Code API client.
///
/// For more information, visit: https://apiverve.com/marketplace/callingcode?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [country] (required) - The 2 letter ISO code for the country (e.g. FR) [minLength: 2, maxLength: 2]
class CallingcodeClient {
  final String apiKey;
  ...
  CallingcodeClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/callingcode',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CallingcodeValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CallingcodeValidationException(errors);
    }
  }

  /// Execute a request to the Country Calling Code API.
  ///
  /// Parameters are validated before sending the request.
  Future<CallingcodeResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CallingcodeException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CallingcodeResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CallingcodeException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CallingcodeException('Resource not found');
      } else {
        throw CallingcodeException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CallingcodeException) rethrow;
      throw CallingcodeException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Country Calling Code API client.
class CallingcodeException implements Exception {
  final String message;

  CallingcodeException(this.message);

  @override
  String toString() => 'CallingcodeException: $message';
}
